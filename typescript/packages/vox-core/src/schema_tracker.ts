...
//
// Receives binette-encoded SchemaPayload from the remote peer, stores
...
import { buildPlan, resolveTypeRef } from "@bearcove/vox-binette";
import { encodeSchemaPayload, decodeSchemaPayload } from "./schema_binette.ts";
...
export class SchemaTracker {
  ...
  /**
   * Record a binette-encoded SchemaPayload from the remote peer.
   */
  recordReceived(
    methodId: bigint,
    direction: BindingDirection,
    binetteBytes: Uint8Array,
  ): void {
    const payload = decodeSchemaPayload(binetteBytes);
...
export interface MethodSendSchemas {
  ...
  argsRootRef: import("@bearcove/vox-binette").TypeRef;
  /** Root TypeRef for the response schema. */
  responseRootRef: import("@bearcove/vox-binette").TypeRef;
}
...
export interface ServiceSendSchemas {
  ...
  schemas: Map<bigint, import("@bearcove/vox-binette").Schema>;
 ...
 * Call `reset()` on reconnect. Call `prepareSchemas()` to get the binette bytes
...
export class SchemaSendTracker {
  ...
  /**
   * Returns binette bytes for the SchemaPayload to embed in a
  ...
  prepareSchemas(
    ...
    const schemas = newDeps
      ...
      .filter((s): s is import("@bearcove/vox-binette").Schema => s !== undefined);
