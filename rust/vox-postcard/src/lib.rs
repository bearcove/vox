...
mod tests {
    ...
    fn translation_bytes_and_list_u8_are_compatible() {
        let remote = SchemaSet::from_schemas(vec![
            vox_types::Schema {
                id: vox_types::SchemaHash(1),
                type_params: vec![],
                kind: vox_types::SchemaKind::Primitive {
                    primitive_type: vox_types::PrimitiveType::U8,
                },
            },
            vox_types::Schema {
                id: vox_types::SchemaHash(2),
                type_params: vec![],
                kind: vox_types::SchemaKind::List {
                    element: vox_types::TypeRef::concrete(vox_types::SchemaHash(1)),
                },
            },
        ]);
        let local_extracted =
    ...
    fn translation_result_with_vox_error_shape() {
        ...
        let r = plan_for(T::SHAPE, T::SHAPE).unwrap();
        let bytes = to_vec(&Ok::<_, VoxError<Infallible>>("hello".to_string())).unwrap();
        let result: T = from_slice_with_plan(&bytes, &r.plan, &r.remote.registry).unwrap();
    ...
    fn translation_result_with_vox_error_err_variant() {
        ...
        type T = Result<String, VoxError<Infallible>>;
        let r = plan_for(T::SHAPE, T::SHAPE).unwrap();
        let bytes = to_vec(&Err::<String, _>(VoxError::<Infallible>::InvalidPayload(
            "bad data".into(),
        )))
        .unwrap();
        let result: T = from_slice_with_plan(&bytes, &r.plan, &r.remote.registry).unwrap();
        match result {
            Err(VoxError::InvalidPayload(msg)) => assert_eq!(msg, "bad data"),
    ...
    fn opaque_round_trip() {
        ...
        eprintln!("outgoing - vox:  {:02x?}", our_bytes);
        ...
        eprintln!("incoming - vox:  {:02x?}", reserialized);
        assert_eq!(
    ...
    fn translation_error_display_coverage() {
        ...
        let dummy_prim = Schema {
            ...
            kind: SchemaKind::Primitive {
                primitive_type: vox_types::PrimitiveType::U32,
            },
        };
