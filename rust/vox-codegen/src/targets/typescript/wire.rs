...
//! - root refs and schema registries for binette-driven protocol codecs
...
/// A wire type to generate TypeScript definitions and binette schema constants for.
pub struct WireType {
    /// The facet Shape to generate from.
    pub shape: &'static Shape,

    /// Prefix for generated schema exports, for example `message` emits
    /// `messageRootRef`, `messageSchemaRegistry`, and `messageSchemas`.
...
/// and binette schema constants. Everything is derived from the shapes.
pub fn generate_wire(config: &WireTypeGenConfig) -> Result<String, Box<dyn std::error::Error>> {
    ...
    // Local canonical schema graphs for binette-driven protocol codecs.
    for root in &config.types {
