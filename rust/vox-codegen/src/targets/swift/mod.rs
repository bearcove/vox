...
/// Generate a complete Swift module for a service, on the phon codec: the named
/// types + the per-service phon schemas/descriptors/programs + the typed client and
/// server. This is the main entry point for Swift code generation.
pub fn generate_service(service: &ServiceDescriptor) -> String {
    ...
    // phon types don't need Codable (the descriptor codec encodes/decodes them; it
    // also can't always synthesize). Drop it like the wire types do.
