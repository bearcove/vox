...
fn codegen_swift(
    ...
    // Validation emit: the phon per-service schema table + descriptors (Stage 3).
    // Panics loudly here name any method shape descriptor_expr can't handle yet.
    {
        let phon = vox_codegen::targets::swift::phon_service::generate_phon_service(testbed);
        let header = "// @generated by vox-codegen — DO NOT EDIT\nimport Foundation\nimport PhonSchema\nimport PhonIR\nimport PhonEngine\nimport VoxRuntime\n\n";
        let out_path = out_dir.join("TestbedPhonSchemas.swift");
        write_if_changed(&out_path, fmt_swift(&out_path, format!("{header}{phon}")))?;

        let client = vox_codegen::targets::swift::phon_client::generate_phon_client(testbed);
        let chdr = "// @generated by vox-codegen — DO NOT EDIT\nimport Foundation\nimport PhonEngine\nimport PhonIR\nimport PhonSchema\nimport VoxRuntime\n\n";
        let cpath = out_dir.join("TestbedClientPhon.swift");
        write_if_changed(&cpath, fmt_swift(&cpath, format!("{chdr}{client}")))?;

        let server = vox_codegen::targets::swift::phon_server::generate_phon_server(testbed);
        let spath = out_dir.join("TestbedServerPhon.swift");
        write_if_changed(&spath, fmt_swift(&spath, format!("{chdr}{server}")))?;
    }
