+++
title = "Wire Format"
description = "The vox-wire value format, its framing modes, and schema-evolution constraints"
weight = 13
+++

`vox-wire` is the single value format used by Vox. It has two framing modes:
...
> r[wire.format]
>
> Vox implementations MUST use `vox-wire` as the value format for protocol
...
> r[wire.modes]
>
> `vox-wire` defines exactly two framing modes:
>
...
> r[wire.mode.self-describing]
>
> Self-describing mode MUST be decodable with zero application schema knowledge.
> A decoder only needs the fixed `vox-wire` tag vocabulary and scalar encodings
...
> r[wire.endianness]
>
> All fixed-width numeric values in `vox-wire` MUST be little-endian.

> r[wire.no-varint]
>
> `vox-wire` MUST NOT use varints for integers, lengths, enum discriminants, or
...
assigned bytes are the permanent bootstrap contract for `vox-wire`.
...
> r[wire.aggregate.struct.self-describing]
>
...
> r[wire.aggregate.struct.compact]
>
...
> r[wire.aggregate.enum.self-describing]
>
...
> r[wire.aggregate.dynamic-value]
>
...
> r[wire.translation.name-matching]
>
...
> r[wire.translation.default-fill]
>
...
> r[wire.translation.no-evolution-fallback]
>
> Skip, default-fill, and reorder are normal schema-evolution operations. A
> native decoder MUST be able to compile all three for compact `vox-wire`
...
> r[wire.bootstrap.self-describing]
>
